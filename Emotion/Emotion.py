...
def playSong():
    ...
    var = 'songs/'+name
    os.system("python MusicPlayer.py " + var)
...
title.config(bg='#FFFF33', fg='red')  
title.config(font=font)           
title.config(height=3, width=80)       
title.place(x=7,y=5)

font1 = ('times', 14, 'bold')
upload = Button(main, text="Upload Image With Face", command=upload, width=30)
upload.place(x=50,y=150)
upload.config(font=font1)  

pathlabel = Label(main)
pathlabel.config(bg='#FFFF33', fg='red')  
pathlabel.config(font=font1)           
pathlabel.place(x=420,y=150)

preprocessbutton = Button(main, text="Preprocess & Detect Face in Image", command=preprocess, width=30)
preprocessbutton.place(x=50,y=200)
preprocessbutton.config(font=font1) 

emotion = Button(main, text="Detect Emotion", command=detectEmotion, width=30)
emotion.place(x=50,y=250)
emotion.config(font=font1) 

emotionlabel = Label(main)
emotionlabel.config(bg='#FFFF33', fg='Red')  
...
playsong = Button(main, text="Play Song", command=playSong, width=30)
playsong.place(x=50,y=300)
playsong.config(font=font1) 


font1 = ('times', 12, 'bold')
text=Text(main,height=10,width=70)
